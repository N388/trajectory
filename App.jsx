...
export default function App() {
  ...
  const liveBidPct  = useRef(50);
  const liveAskPct  = useRef(50);
  ...
  const connectDepth = useCallback(() => {
    ...
    ws.onmessage = e => {
      ...
      liveBidPct.current = bidPct;
      liveAskPct.current = askPct;
      setInfo(d2 => ({ ...d2, bidPct, askPct }));
    };
  ...
  const draw = useCallback(() => {
    ...
    cloudTrajs.current.forEach(traj => {
      ...
      const a = Math.max(0.01, 0.11 - ageSec / 700);
    ...
    if (latest) {
      ...
      ctx.beginPath(); ctx.arc(tx(ep.time), ty(ep.price), 3, 0, Math.PI * 2); ctx.fill();
  ...
  const { price, change24h, bidPct, askPct, connected, error } = info;
  const ch24up = change24h >= 0;
  const isUp   = bidPct >= askPct;
 
  const dotColor   = error ? "#ff5566" : connected ? "#00ff88" : "#ffcc00";
  const statusText = error ? error : connected ? "متصل · بيانات مباشرة مجانية" : "جاري الاتصال...";
  ...
  return (
    <div style={{ background:"#040810", minHeight:"100vh", color:"#c0d0e0", fontFamily:"'Courier New',Courier,monospace", direction:"rtl", display:"flex", flexDirection:"column", alignItems:"center", padding:"14px 10px", gap:10 }}>
      ...
      <div style={{ fontSize:9, color:"#111d2a" }}>
        WebSocket مباشر من Binance · مجاني 100% · منحنى كل 10 ثواني · يمتد 10 دقائق للأمام
      </div>
